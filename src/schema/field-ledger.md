...
| `meta.initialized` | boolean/false | 开场正常生成链 | 模型、开场页 | 建档成功后置 true，不回退 |
| `meta.opening_committed` | boolean/false | 开场事务 | 开场页 | 防重复提交；失败时保持 false |
| `environment` | 日期、时段、季节、天气 | 模型 | 模型、庭园页 | 时段按固定环推进；季节日限制 1–30 |
| `player` | 身份与当前位置 | 开场模型、模型 | 模型、庭园交互 | UI 草稿不得直接写入；位置不生成地图玩家小人 |
| `garden` | 庭园名、建设阶段、锚点引用 | 开场模型、事件模型 | 模型、庭园页 | 临时锚点最多 2 个；引用必须存在 |
...
| `key_items` | 关键物品字典 | 模型 | 模型、UI | 不进入传统背包；关键物不得无因删除 |
