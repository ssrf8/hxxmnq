...
- 玩家在 GAL 壳中阅读的主内容是回复里的**自然叙事正文**；`narration` 与 `dialogue` 本身同时承担文字播放和立绘提示，不再输出第二份表现数据。
...
- 固定推进剧情不输出额外选项、`<w2g>`、`<catsay>` 或玩家可执行指令；
- 结尾后由本地事务结算并返回庭院，玩家不能继续在该剧情输入文字；
- 只有 `interaction.current_session` 明确为自由交流时，界面才允许本地预设回应、手动输入和多轮对话。
...
2. 正文结束后不要再复述正文，也不要输出第二份 GAL 表现 JSON；立绘状态直接取自各个 `dialogue` 的属性。
...
`dialogue` 的 `visual_mode` 只描述当前片段使用哪组本地贴图，不授予行为、同意、关系、事件或变量结果：
...
旧消息仍由界面的本地兼容路径读取，不得要求玩家重开聊天。旧 `dialogue` 缺少 `visual_mode` 时按 `normal` 处理。不要输出任意图片路径；界面根据 `char + visual_mode + reaction + pose` 选择受控素材，缺失时按本地注册表降级到角色默认图或占位图。
