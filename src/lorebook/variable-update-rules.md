...
- `greenhouse_multiturn_conversation` 为单轮固定结算：本地结算器在真实 assistant 回复到达后一次性写入完成标记、幂等结算并清空任何遗留会话。停止生成、失败回复、同楼重放、Swipe 切换和纯格式修复都不计数。
- 玩家若在结算前主动结束，本地只做幂等收尾，不写完成标记；此后可重新发起温室研究交流。
...
- 机遇卡的候选过滤、稳定随机、扣卡、到场与认识事实由本地事务一次提交，全程不调用模型；角色菜单中的「符卡对战」的目标、难度、胜负和杂鱼标签同样由本地独占，不消耗道具。对战难度在开战时按标签锁定：0 枚为原作 Hard 风格极难档、1–2 枚为标准档、3 枚以上为援助档。战后必须先由本地结果弹窗告知标签变化：胜利消除一枚（零枚时保持零），失败增加一枚；标签仅降低后续角色对战强度。对战失败不得创建消息或剧情，只有本地可信胜利结算后才允许向剧情模型投影一次锁定的胜利要求；该独立战后场景允许对手原本不在庭院，出场不等于到场，模型不得重判胜负、拒绝要求、补写 `GensokyoPresence` 或改写任何对战运行字段。
