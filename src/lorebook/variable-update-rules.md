...
- 机遇卡的候选过滤、稳定随机、扣卡、到场与认识事实由本地事务一次提交，全程不调用模型；对战卡的目标、难度、胜负、消耗和杂鱼标签同样由本地独占。对战难度在开战时按标签锁定：0 枚为原作 Hard 风格极难档、1–2 枚为标准档、3 枚以上为援助档。对战失败不得创建消息或剧情，只有本地可信胜利结算后才允许向剧情模型投影一次锁定的胜利要求；该独立战后场景允许对手原本不在庭院，出场不等于到场，模型不得重判胜负、拒绝要求、补写 `GensokyoPresence` 或改写任何卡片运行字段。
