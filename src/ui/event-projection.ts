...
export function buildEventPromptProjection(eventId: string, actionId: string, state: GardenState) {
  ...
  const selectedChoice = event.action_results?.[actionId];
  return [
    '【当前事件精确投影】',
    `事件：${event.config_id}（${event.title}）`,
    `类型：${event.event_type}；最多有效轮数：${event.max_effective_rounds}`,
    `本次行动：${actionId}`,
    list('参与角色 ID', selectedChoice ? [selectedChoice.proposer_id] : event.participants),
    ...
    list('本地允许结果 ID', selectedChoice ? [selectedChoice.result_id] : event.allowed_results),
    selectedChoice
      ? `本次固定选择：提出者 ${selectedChoice.proposer_id}；形态 ${selectedChoice.form_name}；结果 ${selectedChoice.result_id}；固定结尾：${selectedChoice.fixed_ending}`
      : '',
