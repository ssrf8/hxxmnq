...
export function renderShopView(
  ...
  for (const item of listShopItems(state)) {
    ...
    summary.textContent = item.item_type === 'resource'
      ? `物资 +${item.quantity}`
      : item.item_type === 'trigger_item'
      ? `持有 ${state.inventory?.consumables?.[item.item_id] ?? 0}`
      : '唯一关键物品';
    details.append(price, summary);
    ...
    const usable = (item.item_type === 'trigger_item' && item.item_id === 'incident_trigger_card' && (state.inventory?.consumables?.[item.item_id] ?? 0) > 0)
