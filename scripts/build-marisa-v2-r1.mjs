...
const CELL = 209;
const TARGET_H = 150;
const LOW_BOTTOM = 179; // exclusive bottom for low hover pose
const HIGH_RISE = 8;
...
function placeWithScale(png, scale) {
...
const downLowB = contentBBox(keys.downLow);
const commonScale = TARGET_H / downLowB.h;
console.log('commonScale', commonScale.toFixed(4), 'downLow', downLowB);

const base = {
  rightLow: placeWithScale(keys.rightLow, commonScale),
  rightHigh: placeWithScale(keys.rightHigh, commonScale),
  leftLow: placeWithScale(keys.leftLow, commonScale),
  leftHigh: placeWithScale(keys.leftHigh, commonScale),
  downLow: placeWithScale(keys.downLow, commonScale),
  downHigh: placeWithScale(keys.downHigh, commonScale),
  upLow: placeWithScale(keys.upLow, commonScale),
  upHigh: placeWithScale(keys.upHigh, commonScale),
};
...
const HORIZ_T = [0.0, 0.3, 0.6, 1.0, 0.7, 0.35, 0.05, 0.25];
const HORIZ_SWAY = [0, 1, 1, 0, -1, -1, 0, 1];
const VERT_T = [0.0, 0.45, 1.0, 0.45];
const IDLE_T = [0.15, 0.45, 0.75, 0.4];

const dir00 = (low, high) => makeHover(low, high, 0.25, 0);
...
const outPath = path.join(ROOT, 'marisa-animation-v2-r1.png');
...
const stripPath = path.join(ROOT, 'marisa-animation-v2-r1-right-strip.png');
