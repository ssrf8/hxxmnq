...
// 用法: node scripts/export-${CHAR}-sequence-gifs.mjs [--ms=90]
...
const FRAME_COUNT = parseInt(process.argv.find((a) => a.startsWith("--frames="))?.split("=")[1] ?? "28", 10);
const SRC = path.join(ROOT, `src/assets/characters/${CHAR}/sequence-v1`);
...
function main() {
  ...
  for (const d of DIRECTIONS) {
    ...
    const p = path.join(PREVIEW, `${CHAR}-sequence-${d}.gif`);
  ...
  const op = path.join(PREVIEW, `${CHAR}-sequence-overview.gif`);
