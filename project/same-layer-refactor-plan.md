...
> 实施状态（2026-07-23）：同层壳、确定性开场与宿主生命周期已在 `0.2.0-r14` 完成所有者验收。下一阶段的角色 GAL、目标操作与设施事务由 `gal-interaction-plan.md` 接续；本文件继续作为底层同层壳和消息事务依据。
...
确定性开场是唯一例外：新聊天不进入消息生成状态机。它冻结草稿与 chatId，将共享初始状态和开场字段写入已有的首个 assistant 楼层，复读成功后直接进入庭院。普通互动、结算及旧版开场恢复仍沿用真实 user/assistant 消息事务。
...
| 临时事务 | iframe 内存 | 开场写入锁、正在提交、生成 ID、错误 |
...
- 普通互动和旧版修复操作共享消息事务保护；新开场使用独立的零生成 MVU 写入锁。
- 连点不会重复创建消息。
- 已实现生成失败、停止后继续生成和聊天切换保护；失败、停止与切换组合仍需真实运行验收。
- 普通互动的真实 user/assistant 消息与 MVU 状态均正常落盘；新开场不创建消息，必须在首个 assistant 楼层复读确认。
...
- 所有者验证新开场零生成初始化、刷新恢复与第一次真实行动生成；旧开场恢复只做兼容回归。
...
- 重新生成与左右 Swipe。
- 开场零生成提交、重复点击、写入失败保留草稿与旧聊天确定性恢复。
