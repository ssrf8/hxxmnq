...
- 三张旧 V2 损坏覆盖层因画布、透视和锚点不匹配未复用；`fairy-garden-damaged-overlay-v3.png`、`moon-spring-damaged-overlay-v3.png`、`banquet-plaza-damaged-overlay-v3.png` 仍待补齐。
...
- 当前策略：V3 正常形态已启用；未建设施继续显示交互地块标记，损坏状态暂以正式状态标签表达，等待 V3 损坏覆盖层
...
| 妖精花园 | 右上空地 `0.81, 0.36` | `四季花境`、`妖精游乐庭`、`冰露迷宫` | 1 张通用损坏覆盖层 |
| 月见温泉 | 左下空地 `0.22, 0.70` | `露天月见汤`、`静水观测池`、`雾隐汤屋` | 1 张通用损坏覆盖层 |
| 宴会广场 | 右下空地 `0.78, 0.69` | `灯火夜市`、`鬼之大宴台`、`符卡演武场` | 1 张通用损坏覆盖层 |

合计：13 张完整形态图 + 3 张损坏覆盖层 = 16 张。
...
- 损坏层：必须与该设施组正常形态同尺寸、同锚点，只保留裂纹、碎片、焦痕等变化像素。
...
3. 本轮必做只有三张设施损坏覆盖层及其接入：
   - `fairy-garden-damaged-overlay-v3.png`
   - `moon-spring-damaged-overlay-v3.png`
   - `banquet-plaza-damaged-overlay-v3.png`
4. 主屋两张状态层是可选扩展，不得与本轮三张设施覆盖层混做；须等所有者另行确认。
5. 本轮允许离线构建和 checkpoint dry-run，不允许正式打包、覆盖 `0.2.0-r54` 或生成可分发检查点。`battle-bullets-etama3-local-v1.png` 是个人本机限定素材，禁止进入公开打包或分发。

### 1. 制作三张共享损坏覆盖层

1. 按 `imagegen` 技能流程先目视同组全部 V3 正常形态，再查看旧 V2 损坏层；旧 V2 只作损坏语汇参考，禁止直接缩放、重采样后冒充 V3。
2. 每张覆盖层必须能叠加在同组全部三种形态上，优先放置在共同结构和地面接触区，避开只属于某一形态的屋顶、招牌或独占装饰。
3. 建议损坏语汇：
   - 妖精花园：断裂围栏、散落木片、踩乱花圃、少量结构崩边。
   - 月见温泉：池沿裂纹、碎石、断管／漏汽痕迹，避免大面积遮住水面。
   - 宴会广场：地板裂损、碎木／碎石、结界纸带破损、克制的焦痕。
4. 正式文件只能包含变化像素，不得包含完整设施、人物、文字、按钮、地图底色或大面积重复阴影。
5. 若生成过程需要 chroma／中间图，维护源放入 `旧素材/素材处理/` 下的独立子目录；正式产物只放入对应的 `src/assets/world/map-facilities/<group>/`。
...
- 每张覆盖层分别叠加同组三种正常形态，输出或实看 9 个组合；任一组合出现悬空、穿模、遮字／遮人物或损坏位置明显不属于该形态时，先回素材层修正。
- 可扩展 `scripts/prepare-v3-facility-assets.py` 或新增单用途确定性脚本完成尺寸、alpha、隐藏 RGB、边界和哈希报告；不得靠手工反复另存制造不可复现产物。

### 3. 接入范围

所有者确认三张覆盖层视觉通过后再改维护源：

1. 在 `src/assets/asset-manifest.json` 的 `fairy_garden`、`moon_spring`、`banquet_plaza` 条目分别新增精确的 `damage_overlay_alpha` 路径。
2. 将 `maps.garden_base.facility_layer_policy`、三组 `status`、`render_contract` 与 `provenance` 从“damage overlays pending”更新为“V3 damage overlays integrated、real runtime pending”的准确表述。
3. 保持 `src/ui/garden-map.ts` 的既有选择语义：只有 `facility_runtime.<id>.status === 'damaged'` 时叠加覆盖层；`normal`、`abnormal`、未建设和魔法温室不得误叠。
4. `scripts/build-ui.mjs` 已具备覆盖层复制、内嵌和同组画布校验链；优先复用并补强门禁，不另造第二套资源注入路径。
5. 更新 `tests/ui-contract.test.mjs`：
   - 三条 manifest 路径精确断言；
   - 覆盖层与同组正常形态画布一致、RGBA、透明安全边与隐藏 RGB 合格；
   - damaged 状态返回覆盖层，normal／abnormal 不返回；
   - 魔法温室继续无独立损坏覆盖层；
...
1. iframe 中三张覆盖层均由自包含资源链加载，不依赖本机预览相对路径。
2. 三组设施从正常切到 damaged、修复后移除覆盖层，状态标签和实际画面一致。
