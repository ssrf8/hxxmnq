...
- GAL 剧情必须镜像真实 SillyTavern user/assistant 楼层；点击角色的普通“对话”后先进入仅存于 iframe 内存的首轮输入草稿，不自动创建楼层，玩家首次提交后才走正常消息事务；草稿未发送便“结束聊天”只清理 UI 并返回庭院，不调用 LLM、不写消息或 MVU。后续自定义输入和建议回应仍走正常消息事务，已开始会话的普通“结束聊天”只执行本地会话清理，不创建新楼层。
- 回想画廊只从当前聊天最近 1000 个真实 user/assistant 楼层只读投影 GAL 剧情与图片；玩家可按真实楼层号筛选起止范围，并在范围内快速定位 assistant 剧情楼层。画廊不建立第二份剧情、图片或解锁数据库；筛选、浏览、翻页和退出不得写消息或 MVU。
