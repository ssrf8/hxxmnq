...
| 离线门禁 | 当前维护源已通过 `check:ui`、`npm test`（191/191）、`build:ui` 与 `check:assets:r2`；最新 R2 dry-run 为 114 个素材对象、`54,968,893` bytes、manifest SHA-256 `07bf63c2…4cab5fb3`。单桶发布器的受限 staging 测试会生成“素材对象 + 最后发布 manifest”的只读计划，并已证明不存在 live 模式。本轮未重跑角色卡 r54 dry-run、未正式打包，现有 r54 历史产物不覆盖 |
...
| 素材发布 | 当前角色卡继续使用本地 data URL 自包含构建。除 GAL 外的 88 张运行图片已切换 WebP，PNG 维护源保留；R2 本地准备现从活动 manifest 收敛 114 个运行文件、52.42 MiB，其中非 GAL 28.25 MiB，并生成带 SHA-256／MIME／字节的干净、不可覆盖 release staging。所有者已确定整个项目永久只使用一个 R2 桶：核心 release、GAL 滚动池、频道和未来 BGM 通过桶内前缀隔离，共用一份 CORS、一个自定义域名和一个最小权限上传入口。未来上线采用固定 release ID、匿名读取、不可变缓存和双模式回退。当前 `dist/assets` 仍不得整目录上传；真实桶、域名、CORS 应用、上传和远程运行尚未实施或授权 |
...
3. 动手前与收工前各跑一遍离线门禁；若基线已有失败，记录精确测试名与原因，不得把既有失败归到本轮，也不得写成“全绿”。当前离线基线为 178/178，无已知失败。
