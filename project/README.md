...
> 最后整理：2026-08-01（第一版八名固定角色人设、机遇卡／任意角色对战卡、V3 庭园、战斗四状态与 cut-in、14 个 WebAudio 事件音效均已接入。Cloudflare R2 单桶 `hxxwy` 已发布固定 release `0.2.0-r55-bbc0e074f993`，公开 `r2.dev` 预发布、CORS、远程构建和浏览器加载验收通过；114/114 运行素材完成、失败 0。生产自定义域名 `ssrfrrt.ccwu.cc` 尚未绑定，真实 SillyTavern 的角色对话、战斗音频／纸符可读性／四状态／cut-in／妖精动画、宿主缩放和断网回退仍待验收。当前离线门禁为 `check:ui`、`npm test` 191/191、`build:ui` 与 `check:assets:r2` 全绿；未正式打包、未覆盖 r54）。
...
| 离线门禁 | 当前维护源已通过 `check:ui`、`npm test`（191/191）、`build:ui` 与 `check:assets:r2`。R2 release 包含 114 个素材对象、`54,968,893` bytes；加最后发布的 manifest 后远端共 115 个对象、`55,050,924` bytes。正式 release manifest SHA-256 为 `75c79795…83c08e`，source commit 为 `bbc0e074f993`。本轮未重跑角色卡 r54 dry-run、未正式打包，现有 r54 历史产物不覆盖 |
...
| GAL 视觉 | 舞台使用 `gensokyo-gal-shrine-background-v1.png`；预览读取文件、内嵌运行时读取 data URL。旧 CSS 月亮和雾气层已禁用；姓名牌移入带 `clip-path` 的对白框内部，避免被裁掉；桌面及窄屏均保留东方和纸／红漆层级。LLM 表现协议、`scene.v1` 与庭园正文现已接入 `normal / nude / sexual` 三值 `visual_mode`，旧回复默认 `normal`。魔理沙五种反应的 `normal`／`nude` 共 10 张所有者原图已进入 asset manifest、自包含与 remote-r2 双链；当前无 sexual 姿势，按同反应全裸再到普通图降级。R2 浏览器验证已确认 GAL 在非 GAL 素材完成后静默加载；真实 SillyTavern 的逐表情、窄屏和内容设置验收仍待执行 |
...
| 素材发布 | 当前角色卡默认继续使用 data URL 自包含构建；`remote-r2` 必须显式传入固定 release manifest。唯一桶 `hxxwy` 已发布 `0.2.0-r55-bbc0e074f993`，CORS、不可变缓存头、manifest 最后发布和 `r2.dev` 浏览器验收均通过。开场主视觉仍内置；入口只等待 16 项关键素材，非 GAL 完成或 fallback 后再单并发加载 GAL，触发素材可抢占，单项默认三次尝试。生产自定义域名拟使用 `ssrfrrt.ccwu.cc`，当前因 Cloudflare 管理身份未就绪而尚未绑定；`dist/assets` 仍不得整目录上传 |
| 活跃工作线 | ①恢复 Cloudflare 管理登录后把 `ssrfrrt.ccwu.cc` 绑定到 `hxxwy`，等待 ownership/SSL active，再用新域名重建并复验 114/114；完成后决定是否关闭 `r2.dev` ②在真实 SillyTavern 验收首次手势解锁、暂停恢复、音效／BGM 分轨、R2 Canvas/CORS、缓存与断网回退 ③机遇卡、对战卡与杂鱼标签阶段 A–C 已完成，等待正式卡面／小鱼干素材并验收胜败分流 ④按设施清单验收共享废墟、命中和缩放 ⑤R11–R16、弹幕和 M1/M2 集中实机验收欠账 |
...
3. 动手前与收工前各跑一遍离线门禁；若基线已有失败，记录精确测试名与原因，不得把既有失败归到本轮，也不得写成“全绿”。当前离线基线为 191/191，无已知失败。
