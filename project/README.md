...
> 最后整理：2026-07-29（地图、设施、角色序列、三副本 Boss 与入口／选关 UI 已收口；个人本地版追加 etama3 敌弹图集、紧凑可读的顶部 HUD，以及手机主指拖动自动射击。`check:ui`、`npm test` 136/136 与 `build:ui` 全绿，390×844 弹幕演练无溢出。未打包、未覆盖 r54；etama3 本地素材禁止公开打包或分发，真实 SillyTavern 验收仍待执行）。
...
| 离线门禁 | `check:ui`、`npm test`（136/136）与 `build:ui` 全部通过；地图、符卡选关及 `390×844` 手机弹幕演练的离线预览无项目脚本警告／错误。本轮未申请检查点打包，现有 r54 历史产物不覆盖 |
...
| 庭园地图分层 | manifest 已指向所有者提供的 `1448×1086` 新底图 `garden-base-owner-candidate-v2.png`。主屋直接复用底图左上庭院并保留原维修状态与交互，不再叠加旧主屋贴图；魔法温室 4 形态、妖精花园／月见温泉／宴会广场各 3 形态及后三者 `damaged` 覆盖层均使用所有者批准的 `-v2` 正式资源。四座独立设施登记 `width_ratio`、渲染中心、落脚点、标签点和命中多边形；悬停时显示固定中文名并沿透明贴图边缘发光，不再显示内部区域 ID 或已建设施的脚底光圈。离线接入完成，待真实 SillyTavern 验收 |
...
| 活跃工作线 | ①R11–R16 UI／地图在真实 SillyTavern 做缩放、窄屏、案内面板焦点返回、类名改写、键盘与生命周期验收 ②新地图实机复核设施点击、拖动／缩放后菜单跟随、损坏层和宿主裁切 ③弹幕小游戏实机验收与平衡 ④M1/M2 集中实机验收欠账 |
...
3. 动手前与收工前各跑一遍离线门禁；若基线已有失败，记录精确测试名与原因，不得把既有失败归到本轮，也不得写成“全绿”。当前离线基线为 134/134，无已知失败。
