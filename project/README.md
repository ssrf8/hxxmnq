...
> 最后整理：2026-07-29（地图异常续接、设施全形态、七名角色验收序列、三副本独立 Boss 图集与旧素材归档均已收口。前端 R16 将符卡副本／灵梦小店／背包三张缩略入口收拢为「幻想乡案内」单例大面板，入口图改用平滑采样；桌面、390px、320px 离线检查通过。`check:ui`、`npm test` 134/134 与 `build:ui` 全绿。未打包、未覆盖 r54；真实 SillyTavern 验收仍待执行）。
...
| 离线门禁 | `check:ui`、`npm test`（134/134）与 `build:ui` 全部通过；宽屏 1440×1000、窄屏 390×844 离线预览无项目脚本警告／错误。本轮未申请检查点打包，现有 r54 历史产物不覆盖 |
...
| 庭园地图分层 | manifest 已指向所有者提供的 `1448×1086` 新底图 `garden-base-owner-candidate-v2.png`。旧主屋保留损坏／修缮两态回退；魔法温室 4 形态、妖精花园／月见温泉／宴会广场各 3 形态及后三者 `damaged` 覆盖层均使用所有者批准的 `-v2` 正式资源。四座新设施登记独立 `width_ratio`、渲染中心、落脚点、标签点和命中多边形；背景房屋不转为交互对象。离线接入完成，待真实 SillyTavern 验收 |
...
| 弹幕战视觉 | 三副本已分别使用琪露诺、爱丽丝、咲夜四状态 Boss 图集（待机／施法／受击／击破），不再共用温室妖花；透明版进入预览与自包含 data URL 链，chroma 版仅作维护源。琪露诺本地浏览器目视正常；爱丽丝、咲夜和三套图的轻微洋红边缘仍待逐场及真实 SillyTavern 验收。Boss 宣言半身立绘与妖精小怪 sprite 仍为占位／程序化绘制 |
