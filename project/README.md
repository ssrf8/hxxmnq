...
> 最后整理：2026-07-30（角色人设世界书已改为唯一绿灯标记精确触发，姓名／别名与递归扫描不再互相诱发；灵梦、魔理沙、琪露诺、爱丽丝、萃香、荷取六人的人设扩写已经完成本轮收束，米斯蒂娅与咲夜列为下次人设更新目标；V3 横向庭园底图与 13 张正常设施形态已完成差分抠图、同组画布正规化、manifest／运行时接入及桌面／390×844 离线实看；人偶维护型温室的顶部屋脊、右侧人偶区、底部构件，以及河童自动化型温室的顶部管道、左右设备区、底部基座，已按所有者修正标注从原始全景差分确定性恢复。两张修复素材使用 `608×528` 同组画布，所有者已目视验收通过。`check:ui`、`npm test` 141/141、`build:ui` 与 r54 dry-run 全绿。下一步人设工作为米斯蒂娅与咲夜；地图工作为补妖精花园／月见温泉／宴会广场三张 V3 损坏覆盖层并完成视觉复核，之后进入真实 SillyTavern 生命周期、绿灯激活与触控验收。未正式打包、未覆盖 r54；etama3 本机素材禁止公开打包或分发）。
...
| 离线门禁 | `check:ui`、`npm test`（141/141）、`build:ui` 与 `package:checkpoint:dry` 全部通过；V3 庭园桌面／`390×844` 离线预览无项目脚本警告／错误。完成荷取人设后的最新 dry-run 为 `85,383,013` bytes、SHA-256 `e9c266d9…e58fd80`；本轮未正式打包，现有 r54 历史产物不覆盖 |
...
| 庭园地图分层 | manifest 使用所有者提供的 `1672×941` 横向底图 `garden-base-owner-v3.png`，中央主屋直接复用底图。13 张 V3 正常设施形态已按四组共享透明画布接入，并登记独立尺寸、渲染中心、角色落脚点、标签锚点与精确命中多边形；旧 V2 设施及损坏层退出运行时。三张 V3 损坏层和建议的两张主屋状态层仍待补；离线桌面／`390×844` 已实看，真实 SillyTavern 验收未执行。详见 `project/v3-map-facility-asset-checklist.md` |
...
| 弹幕战视觉 | 选关页已改为妖精／森林／结界三卷主题绘卷；三副本分别使用琪露诺、爱丽丝、咲夜四状态 Boss 图集。个人本地版另接入所有者提供的 etama3 敌弹图集，按 shape×hue 绘制并保留几何回退，但该素材明确禁止公开打包或分发。手机主指拖动超过轻触阈值后自动射击，松手停火；轻触、第二指专注、双击 Bomb 与桌面键盘语义保持。390×844 画布约 350×467、弹窗无溢出；爱丽丝／咲夜逐场和真实 SillyTavern 多点触控仍待验收 |
| 活跃工作线 | ①下次人设更新扩写米斯蒂娅与咲夜 ②按 `project/v3-map-facility-asset-checklist.md` 补齐三张 V3 设施损坏覆盖层并完成所有者视觉复核 ③在真实 SillyTavern 复核新地图设施切换、点击、拖动／缩放后菜单跟随、宿主裁切与生命周期 ④R11–R16 其余 UI 宿主验收 ⑤弹幕小游戏实机验收与平衡 ⑥M1/M2 集中实机验收欠账 |
...
3. 动手前与收工前各跑一遍离线门禁；若基线已有失败，记录精确测试名与原因，不得把既有失败归到本轮，也不得写成“全绿”。当前离线基线为 141/141，无已知失败。
