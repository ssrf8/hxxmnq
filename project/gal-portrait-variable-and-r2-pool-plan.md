...
| `sexual` | 独立成人姿势图 | `pose_id` + `act_id` | 只有正文已进入明确成人亲密行为时使用；姿势与行为必须属于该角色白名单 |
...
- 成人体位图片的固定路径、灵梦缺图首发和 R2-only 更新门槛以
...
成人体位图片的固定名称、28 池／角色预留目录、灵梦缺图首发、R2-only 补图流程与首包实现门槛，
统一以 `project/nsfw-pose-live-asset-naming-plan.md` 为准。本节只保留阶段摘要；若与专项文档冲突，
必须先修正文档，不能临时创造另一套文件名。

1. 确认每位允许进入成人卡池的角色名单；未明确确认的角色不得创建 `sexual` 池。
