...
> 2026-08-02 现行修订：`spell_duel_card` 已从商品目录、背包和场景道具槽移除；其后的“对战卡”表述仅保留为历史记录。角色点击菜单现在提供常规「符卡对战」行动，只能向该角色发起。开战前必须以确认弹窗说明本次难度、杂鱼标签规则与“胜利后可提出一项对方必须答应的要求”奖励。战后必先显示本地结果弹窗：胜利移除一枚杂鱼标签（无标签时保持 0），失败添加一枚；弹窗同时说明标签只降低后续角色对战强度。确认后，失败返回庭院，胜利才进入既有胜利要求剧情。旧存档中的该消耗品在迁移时删除，既不折算也不补偿。
...
2026-08-01：对战卡新增角色对话内的上下文入口，并入现有“本轮使用道具”选择弹窗，不新增独立按钮。玩家只能挑战当前交谈角色；现有 `pending_battle`、取消返卡、结算消费、杂鱼标签与胜利要求链保持唯一事实源，不新增 MVU 字段。取消或失败会恢复原对话，胜利继续进入既有要求剧情。离线 `check:ui`、170/170 测试与 `build:ui` 通过；真实 SillyTavern 的对话中发起、取消、失败返回、胜利续接和窄屏触控仍待验收。
