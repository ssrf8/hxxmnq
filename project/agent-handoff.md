...
> 2026-07-30（四十八，机遇卡／对战卡／杂鱼标签进入规划）：所有者决定暂停米斯蒂娅与咲夜人设线，新增两种本地托管消耗品。机遇卡稳定随机邀请一名已完整登记但尚未认识的角色进入庭院，全程不调用 LLM；对战卡允许选择任意完整登记角色进行本地符卡战，不要求已认识或当前在场。对战卡不奖励金币、不推进时段；`clean_win`／`narrow_win` 胜利时杂鱼标签减一并进入一次“对手必须接受玩家一个要求”的真实胜利剧情，`loss` 时只在本地增加一枚以小鱼干表现的“杂鱼标签”，不得创建消息楼层或调用任何模型。标签达到三枚后锁定援助难度，胜利减到两枚后下次恢复标准难度。完整字段、幂等事务、失败回滚、UI、模型投影、测试矩阵和四阶段施工顺序见 `project/opportunity-duel-card-plan.md`。本条与计划只更新文档，**尚未修改功能源码、未构建、未打包**；两张卡与杂鱼标签正式素材由所有者后续提供。
