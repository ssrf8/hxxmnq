...
- 当前维护源已连续完成 R35 三方案选型、R36 特殊道具与 R37 UI/运行韧性候选；R37 精确检查点已正式生成，等待集中完成 R33～R37 的真实新聊天验收。R33/R34 既有独立验收仍未单独宣告通过。
...
- 最新离线门禁：`npm run check:ui`、`npm test` 44/44、`npm run build:ui`、`npm run package:checkpoint:dry` 全部通过。R37 已正式生成于 `../dist/checkpoint-0.2.0-r37/幻想乡物语-测试检查点-0.2.0-r37.json`，大小 `29,991,599` 字节，包含 16 条世界书，SHA-256 为 `e5a0bef539461ef914c78479c62549bd5402ab2b19d2c0feb080bde9b985e0f0`；反向读取确认为 `chara_card_v2` / `2.0`。真实 Luker 验收仍未执行。
- R34 成品与目录必须冻结。本次优化没有正式打包、导入或覆盖 R34；包含新架构的首个候选必须使用新的未占用检查点。
- 所有者已决定 R35～R37 连续推进并在 R37 精确候选集中验收；R35/R36 不再单独正式打包，但各轮离线门禁和待验收项必须保留，集中验收前不得宣告 accepted。
...
- 当前待验收检查点：`0.2.0-r37-m1-release-candidate`。维护源、离线门禁和正式 JSON 打包已完成；导入和集中真实验收尚未执行。集中清单见 `project/r37-acceptance-checklist.md`。
- 实施顺序：导入 R37 精确候选到真实 Luker 新聊天 → 按清单集中验收 R33～R37、O4 世界书激活与时间/区域护栏 → 通过后再把 M1 标记为 complete。
...
2. R37 正式交付前先核对 dry-run 哈希、未占用输出路径和目标 Luker 新聊天；不得把维护源通过当作真实验收。R35 的按钮、结果、成本与结算已登记在事件 JSON，后续不得新增 `current_route` 或恢复模型选结果。
