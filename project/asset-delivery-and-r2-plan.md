...
> 2026-08-01 决策：以下为 `remote-r2` 调度目标合同。当前 `src/ui/asset-preloader.ts` 仍是 114 项
> 单队列并以全量完成作为进入检查；优先队列、场景包、GAL 抢占和可选离线包尚未实现，不能据此
> 宣称运行时已经验收。
...
**2026-08-01 进度：本地准备已完成，远程运行模式尚未接线。** 已新增 manifest 驱动的运行素材收敛器、
不可覆盖的干净 staging 生成器、逐文件 MIME／字节／SHA-256／缓存元数据、路径与维护稿排除门禁、
公开匿名读取 CORS 模板及专项测试。当前预检收敛 114 个运行文件；真实桶、域名和上传尚未执行。
...
前提：所有者明确提供并批准唯一桶、账号范围、自定义域名、部署命令、验证命令和秘密配置入口。
...
本地 A–F 的代码骨架已完成至“需要真实桶坐标与授权”边界：v2 调度清单、优先队列、入口门、场景抢占、GAL 门控、可信 resolver、HTTP cache 复用和可测试的显式离线缓存服务均已落地。离线包设置 UI 与真实 SillyTavern 验收仍未完成；任何 R2 登录、建桶、CORS、域名、上传、删除与 Git 提交/推送均未执行。

发布工具只提供单桶 dry-run 计划，不提供真实执行分支。正式流程必须先在干净提交上生成新的不可覆盖 staging，再显式传入唯一 bucket；审查输出后仍需所有者另行授权，才可新增云端执行实现。
