...
> 记录日期：2026-08-11（首次记录）；2026-08-12 增补并修复 BUG-20~23；2026-08-14 增补并修复 BUG-24，production UI r17 已发布。
...
| BUG-24 GAL 偶发重复 MVU 初始化并显示“没有可显示的正文” | 已修复、实机验收通过并发布 r17 | assistant 持久化丢失初始化元数据且误接管空占位楼层 | 完整继承上一 assistant 的 MvuData；只接管与 `generate()` 正文完全一致的楼层，已有楼层使用更新 API，不再复制或认领空占位 |
